...
Vamos a ver otros componentes de la UI de usuario que nos permitirán introducir datos y realizaremos un ejercicio que los incluya a todos.

## CheckBox

El `CheckBox` es un elemento muy parecido al `Switch` que ya vimos, pero con otra apariencia. Al igual que el `Switch`, necesita un `Text` para mostrar la etiqueta. Puedes ver un completo manual en:
[https://www.develou.com/android-checkbox-en-compose/](https://www.develou.com/android-checkbox-en-compose/)

Recuerda que nosotros utilizaremos la **delegación del estado**.

-----

## Ejercicio 1

1.  Crea un nuevo proyecto llamado `T05OtrosComponentes`.

    [placeholder $imagen.dialogo\_new\_project\_android\_studio]

    Indicando como `Package name` tu nombre, como muestra la imagen.

    [placeholder $imagen.dialogo\_new\_project\_con\_nombre\_y\_paquete]
...
5.  Realiza un commit llamado "Inicio" y un Push para comprobar que se sube correctamente.

6.  Realiza suficientes commits para que se vea la evolución en el tiempo.

7.  Crea un nuevo fichero de Kotlin llamado `Ejercicio1.kt`.

    [placeholder $imagen.crear\_nuevo\_fichero\_kotlin\_ejercicio1]

8.  Crea las funciones Composables para una pantalla de un pedido de pizza en el que:
    a. El pedido base vale **10 €**.
    b. Permite añadir ingredientes a **2 €** cada uno.
    c. Mostrará el total según los ingredientes seleccionados.
    d. Mostrará las imágenes de los ingredientes seleccionados.

    [placeholder $imagen.evolucion\_ui\_pedido\_pizza]

### Especificaciones:

  * La función composable se encuentra en un fichero llamado `Ejercicio1.kt`.

  * Busca unas imágenes en Internet para los ingredientes e impórtalas a tu proyecto en la carpeta `drawable`.

    [placeholder $imagen.recursos\_drawable\_en\_android\_studio]

    [placeholder $imagen.layout\_pedido\_pizza\_con\_anotaciones]
  ...
  * El código tiene que estar documentado.
  ...
  * Puedes probar la función en la `Activity` principal.
    ...
    [placeholder $imagen.codigo\_llamada\_a\_ejercicio1ui\_en\_mainactivity]
...
[placeholder $imagen.componente\_slider\_en\_compose]

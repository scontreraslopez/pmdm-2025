...
![packet_name](img/T03/activity_195722.png)

Ten en cuenta que la plantilla en la versión actual es diferente a la del manual.
...
Finalmente deberas emparejar el dispositivo con tu PC usando **ADB** (Android Debug Bridge). ADB es una herramienta de línea de comandos que permite la comunicación con un dispositivo Android. Se incluye con Android Studio y el SDK de Android.

---

### 📲 Pasos en el móvil (MIUI)
...
Ten en cuenta que la plantilla de creación es diferente. En el último paso le pasamos el `padding` de `Scaffold`.

```kotlin
Ejercicio2Theme {
    Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
        GreetingText(
            mensaje = "Feliz Cumpleaños Maria",
            from = "De Jose",
...
A partir del ejercicio 2, continúa con el codelab para añadir una imagen de fondo.
...
Crea un nuevo proyecto llamado **Ejercicio4** y crea tres ficheros kotlin con las funciones composable que definan los ejercicios del siguiente Codelab. Intenta realizar la tarea sin consultar la solución.
...
Puedes crear un nuevo fichero con el botón derecho sobre el paquete.
